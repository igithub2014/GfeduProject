...
//                                        adapter = new CourseRecyclerAdapter(baseActivity, dataList);
...
////                                            // 获取当天日期
////                                            String dateStr = courseList.get(i).getDate();
////                                            boolean isCurrentDay = ToolUtils.isCurrentDay(dateStr);
////                                            if(isCurrentDay){
////
////                                            }else{
////
...
//                    );
        ...
        for (int i = 0; i < 10; i++) {
            ...
            for (int j = 0; j < 4; j++) {
                ...
                if (j % 2 == 0) {
                    ...
                    courseInfo.setLessonStartTime("09:00");
                    courseInfo.setLessonEndTime("11:00");
        ...
        adapter = new CourseRecyclerAdapter(this, dataList);
        SlideInBottomAnimationAdapter animationAdapter = new SlideInBottomAnimationAdapter(adapter);
        animationAdapter.setDuration(1000);
        recyclerView.setAdapter(animationAdapter);
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                if (view != null) {
                    if (view.getTop() <= mSuspensionHeight) {
                        ...
                        boolean isCurrentDay = ToolUtils.isCurrentDay(dataList.get(mCurrentPosition).getDate());
                        if (isCurrentDay) { // 表示未今日
                            today_Txt.setText("今日");
                            // 课程开始时间
                            String startTime = dataList.get(mCurrentPosition).getLessonData().get(0).getLessonStartTime();
                            // 课程结束时间
                            String endTime = dataList.get(mCurrentPosition).getLessonData().get(0).getLessonEndTime();
                            ...
                            String dateStr = dataList.get(mCurrentPosition).getDate();
                            String[] str = dateStr.split("-");
                            String newStr = str[2] + str[1] + "月";
                            today_Txt.setText(ToolUtils.setTextSize(baseActivity, newStr, 1, 5, R.style.style2, R.style.style3), TextView.BufferType.SPANNABLE);
                            qr_layout.setVisibility(View.GONE);
                        }
                    }
                }
